    ...
    * One key operation on a Node is to return the inner core 2^(N-1) x 2^(N-1)
    ...
    def __init__(self, level, nw, ne, sw, se, really_use_constructor=False):
        if not really_use_constructor:
            raise UsageError(
                "You should call Node.CanonicalNode rather than the "
    ...
    def __eq__(self, other):
        ...
        if id(self) == id(other):
            return True
        return (
            id(self._nw) == id(other._nw)
            and id(self._ne) == id(other._ne)
            and id(self._sw) == id(other._sw)
            and id(self._se) == id(other._se)
        )
    ...
    def Compact(self):
        ...
        while (
            cur._level >= 2
            and cur._nw._nw == zero
            and cur._nw._ne == zero
            and cur._nw._sw == zero
            and cur._ne._nw == zero
            and cur._ne._ne == zero
            and cur._ne._se == zero
            and cur._sw._nw == zero
            and cur._sw._sw == zero
            and cur._sw._se == zero
            and cur._se._ne == zero
            and cur._se._sw == zero
            and cur._se._se == zero
        ):
            cur = self.CanonicalNode(
                cur._level - 1, cur._nw._se, cur._ne._sw, cur._sw._ne, cur._se._nw
            )
            zero = Node.Zero(cur._level - 2)
        return cur
    ...
    def _Forward(self, atLevel=None):
        ...
        if self._level == 2:
            assert atLevel == 2
            countNW = (
                self._nw.Sum(3) + self._ne.SumLeft() + self._sw.SumTop() + self._se._nw
            )
            countNE = (
                self._ne.Sum(2) + self._nw.SumRight() + self._se.SumTop() + self._sw._ne
            )
            countSW = (
                self._sw.Sum(1)
                + self._se.SumLeft()
            ...
            n11 = Node.MergeCenter(self._nw, self._ne, self._sw, self._se)._Forward(
                atLevel=atLevel
            )
    ...
    def __str__(self):
        ...
        return str(
            (self._level, str(self._nw), str(self._ne), str(self._sw), str(self._se))
        )
    ...
    def _OffsetBounds(cls, bounds, level, index):
        ...
        if index == 0:
            ret = (
                bounds[0] + quarter_size,
                bounds[1] + quarter_size,
                bounds[2] - quarter_size,
                bounds[3] - quarter_size,
            )
    ...
    def Draw(self, bounds, draw_func, offset=(0, 0)):
        ...
        if (
            bounds[1] < -inner_size
            or bounds[0] >= inner_size
            or bounds[3] < -inner_size
            or bounds[2] >= inner_size
        ):
            return
...
class World:
    ...
    def _NodeFromPositionsAndBounds(cls, positions, level, bounds):
        ...
        if level == 1:
            ...
            return Node.CanonicalNode(
                level,
                cls._NodeFromPositionsAndBounds(
                    positions, level - 1, cls._InnerBounds(bounds, 0)
                ),
                cls._NodeFromPositionsAndBounds(
                    positions, level - 1, cls._InnerBounds(bounds, 1)
                ),
                cls._NodeFromPositionsAndBounds(
                    positions, level - 1, cls._InnerBounds(bounds, 2)
                ),
                cls._NodeFromPositionsAndBounds(
                    positions, level - 1, cls._InnerBounds(bounds, 3)
                ),
            )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        initial_state = [(1, 0), (0, 0), (0, 1), (-1, 1), (0, 2)]
        # initial_state = [
        #     (-2, -2),
        #     (-2, -1),
        #     (-2, 2),
